---
name: code-reviewer
description: Review uncommitted repository changes for correctness and quality in a spec-and-plan driven workflow. Use when a user asks for review before commit, validating changes against `docs/spec.md`, `docs/plan.md` boundaries/checklists, existing codebase architecture and patterns, code style conventions, and clean code practices.
---
...
Compare diffs to approved planning artifacts (`docs/spec.md` and `docs/plan.md`) and repository patterns, identify issues by severity, and request concrete fixes before re-review.
...
- Read `docs/spec.md` and `docs/plan.md` if present.
- Read only relevant codebase areas to understand architecture and existing patterns.
...
3. Validate against implementation plan.
- Check that changed behavior maps to planned checklist items and phase boundaries in `docs/plan.md`.
- Flag changes that implement out-of-plan scope or violate defined in-scope/out-of-scope boundaries.
- Flag missing work if `docs/plan.md` marks tasks complete but code/tests do not satisfy completion criteria.
- If `docs/plan.md` is missing, state that as a review limitation and continue.
...
- If `docs/spec.md` is missing, state that as a review limitation and continue with plan/code-quality review.
...
Use these levels:
- `Blocking`: Must fix before merge. Correctness, security, data loss, contract breakage, or clear plan/spec violation.
...
- [what aligns with plan boundaries/checklist intent]
...
- Prioritize bugs, behavioral regressions, plan/spec violations, and missing tests over stylistic nitpicks.
...
- Treat absence of `docs/spec.md` or `docs/plan.md` as documented limitations, not a blocker for all review feedback.
