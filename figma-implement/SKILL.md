...
description: "Translate Figma nodes into production-ready UI code with 1:1 visual fidelity using the Figma MCP workflow (design context, screenshots, assets, and project-convention translation). Framework-adaptive: detect and follow the target project's framework, styling system, and component patterns instead of assuming a fixed stack. Trigger when the user provides Figma URLs or node IDs, or asks to implement designs or components that must match Figma specs. Requires a working Figma MCP server connection."
---
...
This skill provides a structured workflow for translating Figma designs into production-ready UI code with pixel-perfect accuracy. It is framework-adaptive, enforces reuse of existing project components by default, and ensures 1:1 visual parity with designs.
...
### Step 5: Discover Project Stack and Reusable Components

Before implementation, inspect the repository to identify what to reuse and how to implement.

**Required discovery workflow:**

1. Detect the project's UI framework and rendering approach from existing code and dependency/config files.
2. Detect the styling system (design tokens, utility classes, CSS modules, styled components, etc.).
3. Locate existing design system/component directories and shared primitives (buttons, inputs, typography, icons, layout wrappers).
4. Reuse existing components by default; extend variants before creating new components.
5. If framework, design system, or reuse target is ambiguous, ask the user which one to use before coding.
...
- Do not assume any specific framework, language, or styling tool; infer from the repository and follow existing conventions
- Replace generated utility/style patterns with the project's preferred utilities or design system tokens
- Reuse existing components (buttons, inputs, typography, icon wrappers) instead of duplicating functionality
- Use the project's color system, typography scale, and spacing tokens consistently
- Respect existing framework patterns for routing, state management, and data fetching
...
- Place UI components in the project's existing component/design-system directories
- Follow the project's component naming and file organization conventions
...
- If no clear design system exists, discover shared UI primitives in the repo and reuse those patterns
- If multiple design systems or UI libraries exist and ownership is unclear, ask the user which one to target
...
- Follow the project's existing typing and documentation conventions
...
Always discover and check existing components before creating new ones. Reuse and extension are the default path.
...
### Issue: Framework or design system is unclear

**Cause:** The repository does not make the UI stack or component ownership obvious.
**Solution:** Pause implementation and ask the user which framework/component system to target.
