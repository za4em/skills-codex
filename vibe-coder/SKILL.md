---
name: vibe-coder
description: Implement the next planned work item from project docs. Use when a user asks to execute development based on `docs/spec.md` and `docs/plan.md`, requiring code changes aligned with the approved spec, existing architecture/style/patterns, reuse of existing components, and automatic marking of completed plan checklist steps.
---
...
Read `docs/spec.md` and `docs/plan.md`, implement the next unchecked checklist step, verify alignment with the spec and repository conventions, and mark that step done.
...
- Find the earliest unchecked checklist item (`- [ ]`) in `docs/plan.md`.
- Treat that item as the active scope.
- Respect boundaries defined for that phase/task in `docs/plan.md`.
...
6. Mark completion.
- Update `docs/plan.md` by converting the completed item from `- [ ]` to `- [x]`.
...
- If no unchecked checklist items remain in `docs/plan.md`, advise the user to clear context/start a new session and run `$code-reviewer`.
...
### Active Plan Step
- [copied checklist item from `docs/plan.md`]
...
### Plan Update
- [state whether the step was marked `- [x]` in `docs/plan.md`]

### Next Item
- [next unchecked checklist item, if any]

### Final Handoff (When Plan Complete)
...
- Do not drift beyond `docs/spec.md` scope or `docs/plan.md` boundaries without user approval.
